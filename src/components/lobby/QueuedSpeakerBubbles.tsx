 ...
 */
export default function QueuedSpeakerBubbles({ debateId, sides }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const load = async () => {
      const [interestRes, inviteRes] = await Promise.all([
        supabase
    ...
    <div className="border border-border rounded-lg p-3 bg-background">
      ...
      <div className="flex flex-wrap gap-2">
        {rows.map((r) => {
          ...
          return (
            <div key={r.user_id} className="inline-flex items-center gap-1.5 bg-accent/40 border border-border rounded-full pl-0.5 pr-2 py-0.5 text-xs font-body">
              {r.avatar_url ? (
