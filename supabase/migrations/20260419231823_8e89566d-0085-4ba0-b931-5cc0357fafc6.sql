...
BEGIN
  ...
  -- Purge stale rows (not seen in 60s) so they don't permanently hold slots
  DELETE FROM public.live_session_participants
  WHERE session_id = _sid
    AND device_id <> _device_id
    AND last_seen_at < now() - interval '60 seconds';

  -- Host always gets slot 1 — claim or reassign
  IF _is_host THEN
    -- Remove any other row currently in slot 1 that's not this host device
    DELETE FROM public.live_session_participants
    WHERE session_id = _sid
      AND speaker_slot = 1
      AND device_id <> _device_id;

    -- Upsert host row at slot 1
    ...
    ON CONFLICT (session_id, device_id) DO UPDATE
      SET speaker_slot = 1,
          ...
          avatar_url = COALESCE(EXCLUDED.avatar_url, public.live_session_participants.avatar_url),
          user_id = COALESCE(EXCLUDED.user_id, public.live_session_participants.user_id),
          last_seen_at = now();
  ...
  -- Non-host: reuse existing row if device already joined
  ...
  -- Find next free slot starting at 2 (slot 1 is reserved for host)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'live_session_participants_session_device_unique'
  ) THEN
    ALTER TABLE public.live_session_participants
    ADD CONSTRAINT live_session_participants_session_device_unique
